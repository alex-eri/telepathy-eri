...
class ChannelInterfaceGroup(dbus.service.Interface):
  ...
  If the protocol provides no reliable way to tell whether the complete
    ...
    @dbus.service.signal('org.freedesktop.Telepathy.Channel.Interface.Group', signature='a{uu}aua{us}')
    def HandleOwnersChangedDetailed(self, Added, Removed, Identifiers):
        """
        Emitted whenever the HandleOwners
        property changes.

        Clients can assume this signal is emitted by the Connection Manager
        if the MemberIdentifiers property exists
    ...
    @dbus.service.signal('org.freedesktop.Telepathy.Channel.Interface.Group', signature='us')
    def SelfContactChanged(self, Self_Handle, Self_ID):
        """
        Emitted whenever the SelfHandle property
        changes.
