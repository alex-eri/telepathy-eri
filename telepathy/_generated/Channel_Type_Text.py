...
class ChannelTypeText(dbus.service.Interface):
    """\
      ...
      Simple one-to-one chats (such as streams of private messages in
        ...
        TargetHandleType
        is Contact. The
        expected way to request such a channel is to set the ChannelType, TargetHandleType,
        and either TargetHandle or TargetID in a call to
        EnsureChannel.
      ...
      Named chat rooms whose identity can be saved and used again later
        ...
        channels with TargetHandleType =
        Room and the
        Group
      ...
      Unnamed, transient chat rooms which cannot be rejoined by their
        ...
        channels with TargetHandleType = None (and hence TargetHandle = 0),
        Group
      ...
      On protocols like MSN where a conversation with a user is actually
        ...
        SHOULD be represented with TargetHandleType =
        Contact. If a third
        participant
      ...
      If a channel of type Text is closed while it has pending messages,
        ...
        Requested = FALSE
        regardless of its previous value; the InitiatorHandle
        and InitiatorID
      ...
      Opaquely-named rejoinable chatrooms (such as Skype rooms) are
        represented using the properties in the Room2
        interface. Instructions and examples of how to request
        such channels are given in said interface's description.
