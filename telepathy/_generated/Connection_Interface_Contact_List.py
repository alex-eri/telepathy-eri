...
class ConnectionInterfaceContactList(dbus.service.Interface):
    ...
    @dbus.service.method('org.freedesktop.Telepathy.Connection.Interface.ContactList', in_signature='', out_signature='')
    def Download(self):
        """
        Download the contact list from the server. If
          DownloadAtConnection is true,
    ...
    @dbus.service.signal('org.freedesktop.Telepathy.Connection.Interface.ContactList', signature='a{u(uus)}a{us}a{us}')
    def ContactsChangedWithID(self, Changes, Identifiers, Removals):
        """
    ...
    def ContactsChanged(self, Changes, Removals):
        ...
        Emitted immediately after
          ContactsChangedWithID, under the same
          circumstances.

        If clients receive this signal without first receiving a
          corresponding ContactsChangedWithID,
          they MUST assume that only this signal will be emitted.
