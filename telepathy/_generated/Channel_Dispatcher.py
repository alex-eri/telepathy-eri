...
class ChannelDispatcher(dbus.service.Object):
    ...
    @dbus.service.method('org.freedesktop.Telepathy.ChannelDispatcher', in_signature='aoxs', out_signature='aoa{o(ss)}')
    def DelegateChannels(self, Channels, User_Action_Time, Preferred_Handler):
        """
        Called by a
        Handler
        ...
        If this method fails, the original
          Handler
          is still handling the channels.

      
        """
        raise NotImplementedError
  
    @dbus.service.method('org.freedesktop.Telepathy.ChannelDispatcher', in_signature='ox', out_signature='')
    def PresentChannel(self, Channel, User_Action_Time):
        """
        ...
        Channel.
         ...
         If Channel is handled, its handler will be asked to present it the user
         (e.g. bring it into the foreground).
